...
# 1. VERIFICAR, INSTALAR E CARREGAR PACOTES ESSENCIAIS
# Lista de pacotes oficiais do CRAN necessários
pacotes <- c("lavaan", "psych", "readxl", "devtools", "semTools", "MVN")

# Instala os pacotes do CRAN que estiverem faltando
...
# Instala o embedR do GitHub se ele não estiver no computador
if (!requireNamespace("embedR", quietly = TRUE)) {
  devtools::install_github("FredPedrosa/embedR")
}

# Carrega todos os pacotes para a sessão
library(embedR)
library(lavaan)
library(psych)
library(readxl)
...
mtr_normality <- as.data.frame(t(matriz_embeddings))
MVN::mvn(mtr_normality, mvn_test = "mardia")$multivariate_normality
